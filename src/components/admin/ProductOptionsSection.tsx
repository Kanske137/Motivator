    ...
    <div className="space-y-5">
      <Card className="p-5 space-y-5">
        ...
        {/* Per-rad: visa bara det produktblock som hör till den här raden.
            Konsoliderade mallar (`is_consolidated`) visar ALLA aktiverade
            produkttyper i samma vy så admin kan justera storlekar/varianter
            för varje typ utan att byta produktrad. */}
        {(() => {
          ...
          const showCanvas = consolidated ? enabled.includes("canvas") : config.product_type === "canvas";
          const showAluminum = consolidated ? enabled.includes("aluminum") : config.product_type === "aluminum";
          const showAcrylic = consolidated ? enabled.includes("acrylic") : config.product_type === "acrylic";
          // We re-export these as render guards via component-level vars below
          (config as unknown as { __showPoster?: boolean }).__showPoster = showPoster;
          (config as unknown as { __showCanvas?: boolean }).__showCanvas = showCanvas;
          (config as unknown as { __showAluminum?: boolean }).__showAluminum = showAluminum;
          (config as unknown as { __showAcrylic?: boolean }).__showAcrylic = showAcrylic;
          return null;
        })()}
        {(config as unknown as { __showPoster?: boolean }).__showPoster && (
