...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { productUid, imageUrl, mockupSceneId } = await req.json();
    console.log("[gelato-mockup] request:", { productUid, imageUrl, mockupSceneId });

    if (!productUid || !imageUrl || !mockupSceneId) {
      return new Response(
        JSON.stringify({
          mockupUrl: null,
          fallback: true,
          error: "productUid, imageUrl and mockupSceneId required",
        }),
    ...
    if (!create || !usedBase) {
      ...
      return new Response(
        JSON.stringify({ mockupUrl: null, fallback: true, error: msg, mockupSceneId }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    const task = await create.json();
    console.log("[gelato-mockup] create body:", JSON.stringify(task).slice(0, 600), "scene:", mockupSceneId);

    if (!create.ok) {
      console.error("[gelato-mockup] create failed", task, "scene:", mockupSceneId);
      const detailMsg = task?.details?.[0]?.message || task?.message || "mockup_unavailable";
      return new Response(
        JSON.stringify({
          mockupUrl: null,
          fallback: true,
    ...
    console.log("[gelato-mockup] polling taskId:", taskId, "on", usedBase, "scene:", mockupSceneId);
    ...
    while (taskId && result.status !== "completed" && result.status !== "failed" && Date.now() < deadline) {
      ...
      try {
        ...
        console.log("[gelato-mockup] poll status:", result.status, "scene:", mockupSceneId);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
    ...
    return new Response(
      JSON.stringify({ mockupUrl, host: usedBase, mockupSceneId, raw: result }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (error) {
