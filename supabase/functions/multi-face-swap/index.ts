...
> {
  ...
  if (res.stage === "start") {
    console.error("[multi-face-swap] Replicate nano-banana error", res.status, res.error);
    if (res.status === 429) {
      return {
    ...
    return {
      ok: false, retriable: res.status >= 500, status: res.status,
  ...
  console.error("[multi-face-swap] Replicate produced no image", res.error);
  return {
