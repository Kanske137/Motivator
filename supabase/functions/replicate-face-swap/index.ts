...
async function runRemoveBackground(params: {
  ...
  const promptText = [
    ...
    `4. CRITICAL EDGE TREATMENT: the subject must NOT have a hard, clean cut-out silhouette against the white background. Instead, the outer edges of the subject (especially the lower body, paws/feet, hair tips, fur, and silhouette extremities) should softly dissolve and feather into loose watercolor washes, gentle pigment bleeds, and a few stray paint droplets that flow organically into the surrounding white space. Think of an aesthetic watercolor portrait where the subject fades out at the edges rather than being masked. Keep the face, eyes and central features crisp and in focus — only the outer/peripheral edges should have this soft watercolor dissolve. No sharp masking artifacts, no visible cut-out outline.`,
    `5. Keep the subject's identity, face, eyes, fur/skin and proportions exactly as in the input photo unless an artistic style is specified below.`,
    styleBlock,
    adminPromptLine,
