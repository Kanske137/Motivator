...
async function runRemoveBackground(params: {
  ...
  const promptText = [
    ...
    `2. Place the subject on a backdrop that is PURE WHITE — exact RGB (255,255,255) / hex #FFFFFF. The background must be perfectly neutral white with ZERO tint: no cream, no beige, no ivory, no off-white, no warm/cool cast, no subtle gradient, no paper texture, no vignette, no shadow halo around the subject. If you generate any tint at all, it is wrong. The white must match a blank web page background exactly so it blends seamlessly with a #FFFFFF web layout.`,
